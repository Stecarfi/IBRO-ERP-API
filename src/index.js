...
// Fallback para rutas que no correspondan a la API
...
app.get('/api/db', authenticateToken, async (req, res) => {
  ...
  try {
    ...
    // Cotizaciones Externas (Ventas Externas de Campo)
    ...
    // Pedidos Externos (Ventas Externas de Campo)
...
app.post('/api/db/sync', authenticateToken, async (req, res) => {
  ...
  // 🛡️ RBAC: Verificación estricta de privilegios para mutaciones críticas
  const callerRoleId = String(req.user?.roleId || '');
  const isMasterOrAdmin = callerRoleId === '1' || (req.user?.cargo && /admin/i.test(req.user.cargo));

  if (!isMasterOrAdmin) {
    if (diff.roles && ((diff.roles.upserted && diff.roles.upserted.length > 0) || (diff.roles.deleted && diff.roles.deleted.length > 0))) {
      return res.status(403).json({ error: 'Acceso denegado: Se requieren permisos de Administrador para modificar roles del sistema.' });
    }
    if (diff.users && diff.users.deleted && diff.users.deleted.length > 0) {
      return res.status(403).json({ error: 'Acceso denegado: Se requieren permisos de Administrador para eliminar usuarios.' });
    }
  }
    ...
    await prisma.$transaction(async (tx) => {
      ...
      const findOrCreateClienteExterno = async (item, comercialId) => {
        ...
        if (item.clienteExternoId) {
          client = await tx.clienteExternoCampo.findUnique({ where: { id: String(item.clienteExternoId) } });
        }
        if (!client && item.clienteId) {
          client = await tx.clienteExternoCampo.findUnique({ where: { id: String(item.clienteId) } });
        }
        if (!client && (item.docCli || item.clienteNit)) {
          const doc = String(item.docCli || item.clienteNit).trim();
          client = await tx.clienteExternoCampo.findFirst({ where: { nitDoc: doc } });
        }
        if (!client && (item.clienteNombre || item.cliente)) {
          const nom = String(item.clienteNombre || item.cliente).trim();
        ...
        if (!client) {
          ...
          client = await tx.clienteExternoCampo.create({
            data: {
              ...
              comercialId: comercialId || req.user?.id || '1',
      ...
      // 2. Clientes
      if (diff.clientes) {
        if (diff.clientes.deleted && diff.clientes.deleted.length > 0) {
          const delIds = diff.clientes.deleted.map(id => id.toString());
          await tx.visitaCampo.updateMany({
            where: { clienteId: { in: delIds } },
            data: { clienteId: null }
          });
          await tx.seguimientoCampo.updateMany({
            where: { clienteId: { in: delIds } },
            data: { clienteId: null }
          });
        }
      ...
      // 21. Cotizaciones Externas (Ventas Externas de Campo aisladas)
      ...
      // 22. Pedidos / Ventas Externas (Ventas de Campo aisladas)
      ...
      if (diff.clientes_externos) {
        ...
        for (const item of diff.clientes_externos.upserted || []) {
          ...
          const cleanItem = sanitizeBackendForPrisma('clienteExternoCampo', item);
          const { id: idToUpsert, ...updateData } = cleanItem;
          ...
          await tx.clienteExternoCampo.upsert({
            where: { id: idToUpsert || item.id },
            update: data,
            create: { id: idToUpsert || item.id, ...data }
          });
        }
      }
